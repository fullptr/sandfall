...
auto move_offset(world& w, glm::ivec2& pos, glm::ivec2 offset) -> bool
{
    ...
    for (int i = 0; i != steps; ++i) {
        ...
        w.wake_chunk_with_pixel(next_pos);
        std::swap(w[pos], w[next_pos]);
        pos = next_pos;
        set_adjacent_free_falling(w, pos);
    }

    if (start_pos != pos) {
        w[pos].flags[is_falling] = true;
        w.wake_chunk_with_pixel(pos);
        return true;
    }
...
inline auto update_pixel_position(world& w, glm::ivec2& pos) -> void
{
    ...
    const auto after_position_update = scope_exit{[&] {
        w[pos].flags[is_falling] = pos != start_pos;
        if (pos == start_pos && properties(w[pos]).gravity_factor) {
            w[pos].velocity = glm::ivec2{0, 1}; // will always try to move at least one block
        }
    }};
...
auto world::wake_chunk_with_pixel(glm::ivec2 pixel) -> void
{
    ...
    if (pixel.x != width_pixels - 1 && (pixel.x + 1) % sand::config::chunk_size == 0)
    {
        ...
        if (valid(neighbour)) { d_chunks[get_chunk_index(d_width, neighbour)].should_step_next = true; }
    }
